![Odin Project](https://img.shields.io/badge/The_Odin_Project-Progress-orange?style=for-the-badge&logo=theodinproject&logoColor=white)
![UFMS SI](https://img.shields.io/badge/UFMS-Sistemas_de_Informação-blue?style=for-the-badge&logo=university&logoColor=white)
![Analista de Sistemas](https://img.shields.io/badge/Role-Analista_de_Sistemas-purple?style=for-the-badge&logo=micro-strategy&logoColor=white)
# 🚀 Dev Journey

Olá! Este repositório é o meu espaço dedicado a documentar minha evolução técnica diária, desafios superados e a consolidação de conhecimentos acadêmicos e práticos.

## 📍 Onde estou agora?
[![Log Atual](https://img.shields.io/badge/Acompanhe_meu_mês-Março_2026-brightgreen?style=for-the-badge&logo=googlesheets&logoColor=white)](./logs/2026/marco.md)

> Clique no botão acima para ver o que estou estudando **hoje**!


## 🎓 Contexto Acadêmico e Profissional
* **Curso:** Bacharelado em Sistemas de Informação (7º Semestre)
* **Instituição:** Faculdade de Computação (FACOM) — UFMS
* **Papel Atual:** Analista de Sistemas

## 🛠️ Stack em Evolução
Minha jornada de estudos atual foca em desenvolvimento, dados e segurança:

* **Desenvolvimento Web:** Full Stack via **The Odin Project** (JavaScript, HTML, CSS)
* **Linguagens:** Java, Python, C, SQL
* **Dados e Infra:** Microsoft Power BI, Redes de Computadores e Cisco Packet Tracer
* **Segurança:** Fundamentos de Segurança da Informação


## 📅 Registro de Aprendizado
Utilizo este log para manter a consistência e a transparência do meu progresso:

| Data | Categoria | O que aprendi? | Status |
| :--- | :--- | :--- | :--- |
| 17/03 | Git/GitHub | Autocommit, `.gitignore` e configuração de e-mail global | ✅ Concluído |
| 15/03 | Web Dev | Manipulação de caminhos relativos no projeto *Odin Recipes* | 🛠️ Em Progresso |


## 📸 Além do Código
Também sou entusiasta de **fotografia e videografia**, o que me ajuda a manter o olhar atento aos detalhes e à estética visual em meus projetos de software.


## 📫 Vamos nos conectar?
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lhbarboz)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/lhbarboz)